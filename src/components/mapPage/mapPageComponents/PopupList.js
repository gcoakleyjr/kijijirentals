...
const PopupList = ({ list, handleDrawerOpen }) => {



    const rentalItems = list.map((item, i) => {
        return (
            <Box key={i} onClick={handleDrawerOpen} className="list-item" sx={{ height: '80px', display: 'flex', flexDirection: 'row', width: '100%', cursor: 'pointer' }}>
